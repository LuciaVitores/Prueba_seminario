...
En 2020 continúa el descenso de la natalidad. Podemos observar que en el último mes del año __Andalucía__ registra un valor cercano a los 4500 puntos, __Cataluña__ presenta cerca de 4000 puntos y __Madrid__ a penas supera los 3000 nacimientos.
...
Para cumplir el objetivo 4 vamos a obtener la media de las natalidades de cada año para poder compararla con mayor facilidad con los valores del PIB que mostraremos a continuación.
...
nuevaNatalidad <- natalidad %>%
  ...
  summarise(nat_prom  = mean(Total))
nuevaNatalidad
```

```{r}
ggplot(data = nuevaNatalidad, aes(x = `Comunidades autónomas`, y = nat_prom)) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  geom_point()
```
...
DT::datatable(PIB2)
```
Hemos cambiado el nombre de la columna de _Comunidades y ciudades autónomas_ por _Comunidades autónomas_ para poder tener una columna con el mismo nombre en las dos tablas que queramos unir en el join. De esta forma cumplimos con el objetivo 4. 
...
ggplot(data = PIB2) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  geom_bar(
    mapping = aes(x = `Comunidades autónomas`, fill = `Comunidades autónomas`),
    width = 2
  )+
